...
const MainContent = ({ onClose }: { onClose: () => void }) => (
  <div className="absolute bottom-32 w-full px-[5%]">
    <div className="grid relative" style={{ gridTemplateColumns: "1fr 500px" }}>
      <NavigationLinks items={MAIN_NAV_ITEMS} onClose={onClose} />
      <MenuCard onClose={onClose} />
    </div>
  </div>
);

const NavigationLinks = ({ items, onClose }: { items: typeof MAIN_NAV_ITEMS; onClose: () => void }) => (
  <div className="pl-4 flex flex-col justify-end">
    {items.map((item, index) => (
      <NavLink key={index} {...item} index={index} dotEffect={true} onClick={onClose} />
    ))}
  </div>
);
